...
import static com.aconst.spinareg.profile.ProfileActivity.PREF_SUBSCRIBE;
...
public class SessionListAdapter extends RecyclerView.Adapter<SessionListAdapter.ViewHolderSession> {
    ...
    private List<SessionItemRealm> sessionItemRealms;
    private int date_type;

    public SessionListAdapter(List<SessionItemRealm> sessionItemRealms, int date_type) {
        this.sessionItemRealms = sessionItemRealms;
        this.date_type = date_type;
    }

    public void setSessionItemRealms(List<SessionItemRealm> sessionItemRealms) {
        this.sessionItemRealms = sessionItemRealms;
    }
    ...
    public void onBindViewHolder(ViewHolderSession holder, int position) {
        SessionItemRealm item = sessionItemRealms.get(position);
        ...
        switch (holder.getItemViewType()) {
            case SESSION_TIMEOUT :
                holder.tvSession.setText(item.getSpecComment());
            case SESSION_EMPTY :
    ...
    public SessionItemRealm getItem(int position) {
        if (position > -1)
            return sessionItemRealms.get(position);
        else
            return null;
    }
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                ...
                case R.id.btnAddToGooCal :
                    if (prefsHelper.getPref(PREF_SUBSCRIBE).equalsIgnoreCase("pro"))
                        exportToGoogleCalendar(context, id);
                    else
                        Toast.makeText(context,
                                "Функция экспорта сеансов в Google календарь доступна "
                                + "только при подписке Pro",
                ...
                case R.id.layEmpty :
                    if (itemRealm.getId() == 0) {
                        ...
                        if (itemRealm.getDuration() < 24 * 60) {
                            ...
                            // Изменить отпуск
                            intent = new Intent(context, VacationsActivity.class);
                ...
                case R.id.ivPhone :
                    Client client = new ClientsController()
                            .getClient(itemRealm.getClientId()).blockingGet();
                    client.makeCall(context);
                    break;
            }
